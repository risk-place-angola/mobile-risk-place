...
final alertWebSocketProvider = Provider((ref) {
  ...
  return AlertWebSocketService(settingsManager: settingsManager);
});
...
class AlertWebSocketService with WidgetsBindingObserver {
  ...
  AlertWebSocketService({SettingsManager? settingsManager})
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      developer.log('App paused, disconnecting WebSocket',
          name: 'AlertWebSocketService');
      disconnect();
    } else if (state == AppLifecycleState.resumed) {
      developer.log('App resumed, reconnecting WebSocket',
          name: 'AlertWebSocketService');
  ...
  void connect({
    ...
    if (authToken == null && (deviceId == null || deviceId.isEmpty)) {
      developer.log('No authentication credentials provided',
          name: 'AlertWebSocketService');
  ...
  void _tryConnect(
    ...
    if (attempt >= 5) {
      developer.log('Failed to connect after 5 attempts',
          name: 'AlertWebSocketService');
    ...
    try {
      ...
      if (onAlert != null) onAlertReceived = onAlert;
      if (onNearbyUsers != null) onNearbyUsersReceived = onNearbyUsers;
      ...
      try {
        ...
        developer.log('Error in onConnected callback: $e',
            name: 'AlertWebSocketService');
      }
    } catch (e) {
  ...
  void updateLocation(
    ...
    try {
      if (_channel == null) {
        developer.log('Cannot update location: WebSocket not connected',
            name: 'AlertWebSocketService');
        return;
      }
      ...
      final jsonMessage =
          jsonEncode(locationMessage.toJson((data) => data.toJson()));

      _channel?.sink.add(jsonMessage);
      developer.log('Location update sent: ($latitude, $longitude)',
          name: 'AlertWebSocketService');
    } catch (e) {
    ...
    _locationUpdateTimer = Timer.periodic(
      ...
      (timer) {
        try {
          ...
          developer.log('Error in location update callback: $e',
              name: 'AlertWebSocketService');
        }
      },
    );
  ...
  void stopLocationUpdates() {
    ...
    developer.log('Stopped automatic location updates',
        name: 'AlertWebSocketService');
  }
  ...
  void _handleMessage(String message) {
    try {
      ...
      if (event == 'nearby_users') {
        ...
        if (_shouldProcessAlert(alertData)) {
          developer.log('Alert received: ${alertData['message']}',
              name: 'AlertWebSocketService');
          try {
            onAlertReceived?.call(alertData);
          } catch (e) {
            developer.log('Error in onAlertReceived callback: $e',
                name: 'AlertWebSocketService');
          }
        } else {
          developer.log('Alert filtered by settings',
              name: 'AlertWebSocketService');
        }
      } else if (type == 'error') {
        final errorMessage = decodedMessage['message'] as String;
        developer.log('Server error: $errorMessage',
            name: 'AlertWebSocketService');
        try {
          onError?.call(errorMessage);
        } catch (e) {
          developer.log('Error in onError callback: $e',
              name: 'AlertWebSocketService');
        }
      } else if (event == 'location_updated' ||
          event == 'location_update_failed') {
        return;
      } else {
        developer.log('Unknown message type: $type, event: $event',
            name: 'AlertWebSocketService');
      }
    } catch (e) {
      ...
      try {
        ...
        developer.log('Error in onError callback: $callbackError',
            name: 'AlertWebSocketService');
      }
    }
  }
  ...
  void _handleNearbyUsers(Map<String, dynamic> data) {
    try {
      ...
      developer.log('Received ${users.length} nearby users',
          name: 'AlertWebSocketService');

      try {
        onNearbyUsersReceived?.call(users);
      } catch (e) {
        developer.log('Error in onNearbyUsersReceived callback: $e',
            name: 'AlertWebSocketService');
      }
    } catch (e) {
      developer.log('Error parsing nearby users: $e',
          name: 'AlertWebSocketService');
    }
  }
  ...
  void _onDone(String? token, String? deviceId, int attempt) {
    ...
    try {
      ...
      developer.log('Error in onDisconnected callback: $e',
          name: 'AlertWebSocketService');
    }
  ...
  Future<void> _handleReconnect(
    ...
    if (attempt < 5) {
      ...
      developer.log(
          'Retrying connection in ${delay}s (attempt ${attempt + 1}/5)',
          name: 'AlertWebSocketService');
  ...
  bool _shouldProcessAlert(Map<String, dynamic> alertData) {
    if (_settingsManager == null || _lastKnownPosition == null) {
      return true;
    }

    final severity = alertData['severity'] as String? ?? 'low';
    final latitude = alertData['latitude'] as double?;
    final longitude = alertData['longitude'] as double?;
    ...
    return _settingsManager!.shouldNotifyAlert(severity, distance);
  }
  ...
  void disconnect() {
    try {
      ...
      if (_channel != null) {
        ...
        developer.log('Disconnected from WebSocket',
            name: 'AlertWebSocketService');
        try {
          onDisconnected?.call();
        } catch (e) {
